#!/usr/bin/env python3
"""
Enrich Arecaceae genome data with Wikidata taxonomy (subtribe to subfamily).
"""
...
def search_wikidata_taxon(scientific_name):
    ...
    try:
        ...
        if data.get("search"):
            # Return the first result that looks like a species
            for result in data["search"]:
                description = result.get("description", "").lower()
                if "species" in description or "plant" in description:
                    return result["id"]
...
def extract_taxonomy_from_lineage(lineage):
    ...
    Returns:
        Dict with family, subfamily, tribe, subtribe, genus
    """
    taxonomy = {
        "family": "",
        "subfamily": "",
...
def query_wikidata_taxonomy(genus, species):
    """
    Query Wikidata for complete taxonomy information.
...
def enrich_arecaceae_data(input_file, output_file):
    """
    Read Arecaceae assemblies and enrich with Wikidata taxonomy.
    ...
    for i, assembly in enumerate(assemblies, 1):
        ...
        if taxonomy:
            ...
            # Add genus_wikidata for consistency with original script
...
def extract_all_taxa(enriched_data, output_file):
    ...
    with open(output_file, 'w') as f:
        f.write("# Higher Taxa in Arecaceae (from Wikidata)\n")
...
def main():
    ...
    print("="*60)
    print("Enriching Arecaceae genome data with Wikidata taxonomy")
    print("="*60)
