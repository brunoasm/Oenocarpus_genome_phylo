...
Output Files (saved in aliscore_output/aliscore_[alignment]/):
...
# Create dedicated aliscore_output folder and subdirectory for this alignment
ALISCORE_BASE_DIR="aliscore_output"
mkdir -p "${ALISCORE_BASE_DIR}"
OUTPUT_DIR="${ALISCORE_BASE_DIR}/aliscore_${ALIGNMENT_NAME}"
mkdir -p "${OUTPUT_DIR}"
...
# Add script directory to Perl's include path so it can find Aliscore_module.pm
